...
| `bayyin/panel.py` | دائرة الاستئناف الثلاثية (جولةٌ عمياء ثم مداولة) |
| `bayyin/rules.py` | قواعد الإجراء الحتمية (تدقيق، قابلية اعتراض، م.200) |
| `bayyin/llm.py` | غلاف Responses API + النموذج الوهمي + التخزين التدريجي |
| `bayyin/tools.py` | بُناة أدوات `file_search` / `web_search` |
| `bayyin/prompts.py` · `config.py` · `settings.py` | المطالبات · إعدادات التشغيل · الإعدادات المركزية |
| `bayyin/state.py` · `timeline.py` · `audit.py` | حالة الدعوى · الوعي الزمني · سجلّ التدقيق |
| `server.py` · `web/index.html` | خادم البثّ · الواجهة التفاعلية (لوحةٌ بصرية + تقويم + إعدادات) |
